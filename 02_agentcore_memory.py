...
REGION = "us-east-1"
MEMORY_ID = "custome_care-38iGtLAaBJ"
...
class MemoryMiddleware(AgentMiddleware):
    ...
    def pre_model_hook(self, state: AgentState, config: RunnableConfig, *, store: BaseStore):
        ...
        """
        try:
            # Safely extract configurable parameters
            configurable = config.get("configurable", {}) if isinstance(config, dict) else {}
            actor_id = configurable.get("actor_id", "default-user")
            thread_id = configurable.get("thread_id", "default-session")
            ...
            print(f"Error in pre_model_hook: {e}")
            import traceback
            traceback.print_exc()
                ...
                for msg in reversed(messages):
                    if isinstance(msg, AIMessage) and hasattr(msg, 'content'):
                        try:
                            ...
                            print(f"Error saving AI message to memory: {e}")
                        break
        except Exception as e:
...
def agent_invocation(payload, context):
    ...
    try:
        ...
        if isinstance(payload, dict):
            query = payload.get("prompt") or payload.get("message") or payload.get("input")
        ...
        if not query or query.strip() == "":
            return {"result": "Error: No prompt found in input", "error": "missing_prompt"}
        
        # Extract actor_id and session_id from context (RequestContext object)
        # Access attributes directly since context is a RequestContext object, not a dict
        actor_id = "default-user"
        session_id = "default-session"
        
        if context:
            # RequestContext has attributes: session_id, actor_id, etc.
            try:
                session_id = getattr(context, 'session_id', None) or "default-session"
                actor_id = getattr(context, 'actor_id', None) or "default-user"
            except AttributeError:
                # Fallback: try to access as dict if it has __getitem__
                try:
                    session_id = context.get("session_id", "default-session") if hasattr(context, 'get') else "default-session"
                    actor_id = context.get("actor_id", "default-user") if hasattr(context, 'get') else "default-user"
                except:
                    pass
        
        # Fallback to payload if not in context
        if actor_id == "default-user" and isinstance(payload, dict):
            actor_id = payload.get("actor_id", "default-user")
        if session_id == "default-session" and isinstance(payload, dict):
            session_id = payload.get("thread_id") or payload.get("session_id", "default-session")
        
        # Configure memory context
        config = {
            "configurable": {
                "thread_id": session_id,  # Maps to AgentCore session_id
        ...
        if not messages:
            return {"result": "No response generated", "error": "empty_response"}
        ...
        if hasattr(last_message, 'content'):
            ...
            answer = last_message.get("content", "No content in response")
        else:
        ...
        print(f"Error in agent_invocation: {e}")
        import traceback
        traceback.print_exc()
        return {
